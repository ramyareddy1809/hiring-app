pipeline {
    ...
    stages {
        stage('Docker Build') {
            steps {
                sh "docker build . -t ramyabojjala1809/hiring-app:$BUILD_NUMBER"
            }
        }
        stage('Docker Push') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'docker-token', usernameVariable: 'hubUser', passwordVariable: 'hubPwd')]) {
